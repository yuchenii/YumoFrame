...
class NormalizeResultsTest(unittest.TestCase):
    ...
    def test_splits_punctuated_text_using_character_timestamps(self) -> None:
        segments = split_timestamped_text(
            "你好，世界。", [[0, 100], [100, 200], [300, 400], [400, 500]]
        )
    ...
    def test_ascii_words_consume_one_timestamp(self) -> None:
        segments = split_timestamped_text(
            "对战model和Henry。",
            [[0, 100], [100, 200], [200, 400], [400, 500], [500, 700]],
        )

        self.assertEqual(segments[0]["text"], "对战model和Henry。")
        self.assertEqual(segments[0]["end"], 0.7)

    def test_alignment_ignores_unspoken_em_dashes_when_splitting_sentences(
        self,
    ) -> None:
        text = "第一句。不是这样——是那样。"
...
class AlignManifestTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manifest.write_text(
                json.dumps(
                    {
                        "version": 1,
                        "items": [{"id": "intro", "audio": "missing.wav", "text": "你好"}],
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_aligns_ordered_items_with_one_model(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                [item["transcript"]["segments"][0]["text"] for item in payload["items"]],
                ["你好", "世界"],
            )
            self.assertTrue(
                all(item["valid"] and item["issues"] == [] for item in payload["items"])
            )
            auto_model.assert_called_once_with(model="fa-zh", device="cpu", hub="hf")
...
class MainDispatchTest(unittest.TestCase):
    def test_positional_asr_route_still_dispatches(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            transcribe.assert_called_once_with(
                source.resolve(), "cpu", "", 30000, None, "modelscope"
            )
    ...
    def test_positional_align_route_still_dispatches(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            align.assert_called_once_with(source.resolve(), "你好", "cpu", None, "modelscope")
            write_outputs.assert_called_once_with(payload, source.with_suffix("").resolve())
