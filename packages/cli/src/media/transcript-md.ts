 ...
 */
import type { Transcript, TranscriptSegment } from "../core/types.ts";
 ...
 */
export function applyTranscriptMd(
  transcript: Transcript,
  parsedSegments: ParsedTranscriptSegment[],
): Transcript {
  ...
  if (parsedSegments.length !== existing.length) {
    throw new Error(
      `transcript.md has ${parsedSegments.length} segments but json has ${existing.length}`,
    );
  }
  ...
  const segments = existing.map((segment, index) => {
    ...
    if (
      Math.abs(edited.start - segment.start) > 0.051 ||
      Math.abs(edited.end - segment.end) > 0.051
    ) {
      throw new Error(
        `segment ${index} time mismatch: md [${edited.start}-${edited.end}] vs json [${segment.start}-${segment.end}]`,
      );
    }
    return {
      ...segment,
