...
export function buildCharTimeline(
  ...
  cleanedSegments: TranscriptSegment[] | null = null,
): TimelineChar[] {
...
function matchWithSkips(
  ...
  for (let start = from; start < significantTimeline.length; start += 1) {
    ...
    while (needleIndex < needle.length && at < significantTimeline.length) {
      ...
      for (
        let ahead = at + 1;
        ahead < significantTimeline.length && skipped < maxSkip;
        ahead += 1
      ) {
        skipped += 1;
 ...
 */
export function alignStoryboardLines<T extends AlignLine>(
  lines: T[],
  timeline: TimelineChar[],
): { lines: T[]; warnings: string[] } {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    if (!span) {
      ...
      aligned.push({
        ...line,
        start: round(fallbackStart),
        end: round(Math.max(fallbackEnd, fallbackStart + 0.05)),
      });
      continue;
    }
