 ...
 */
export function transcribeInvocation({
  root,
  config,
  outputBase,
}: {
  ...
  if (asr.runner === "command") {
    ...
    if (asr.command.length === 0)
      throw new Error("processors.asr.command must be a non-empty array");
    return {
      command: asr.command[0]!,
      args: [...asr.command.slice(1), mediaPath, outputBase],
      env: { ...process.env, ...asr.env },
    };
  }
  if (asr.runner === "api")
    throw new Error(
      'ASR via the api runner is not supported; use runner "uv" (funasr) or "command"',
    );
...
export async function transcribeProject(
  start = process.cwd(),
): Promise<{ transcriptPath: string; transcriptMdPath: string; voicePath: string }> {
  ...
  const outputBase = resolve(
    ...
    `yf-tmp-${basename(transcriptPath, extname(transcriptPath))}-${process.pid}-${Date.now()}`,
  );
  ...
  const temporaryVoice = resolve(
    dirname(voicePath),
    `yf-tmp-${basename(voicePath, extension)}-${process.pid}${extension}`,
  );
  await run("ffmpeg", [
    "-hide_banner",
    "-loglevel",
