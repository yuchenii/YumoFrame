...
import { CONFIG_FILE, loadConfig } from "../core/config.ts";
import { parseTranscript } from "../core/json.ts";
import { PACKAGE_ROOT } from "../core/package-root.ts";
import { formatTranscriptMd } from "../media/transcript-md.ts";
import {
  fillArgs,
  optionFlags,
  ...
  run,
  runApiSpeech,
} from "../media/processor-runtime.ts";
import { subtitlesToTranscript } from "../media/subtitles.ts";
import { transcribeInvocation } from "./transcribe.ts";
import {
  parseSpeechPlan,
...
export type SynthesizeTimingMode =
  | "native"
  | "fragment-align"
  | "whole-align"
  | "asr-fallback"
...
function lastTranscriptTimestamp(transcript?: Transcript): number | null {
  ...
  const timestamps = transcript.segments
    .map((segment) =>
      segment.timestamp?.at(-1)?.[1] !== undefined
...
function synthesisResult({
  ...
  return {
    ...
    lastTimestamp,
    coverage:
      lastTimestamp === null || duration === 0
        ? 0
        : Math.round((lastTimestamp / duration) * 1_000_000) / 1_000_000,
...
function uvOptions(config: YumoFrameConfig, root?: string): Record<string, string | number> {
  ...
  if (
    ...
    !isAbsolute(options.refAudio) &&
    !/^[a-z][a-z\d+.-]*:/i.test(options.refAudio)
  ) {
...
export function synthesizeInvocation({
  config,
  text,
  outPath,
  root,
  ...
  const tts = config.processors.tts;
  if (!tts || tts.runner !== "uv") throw new Error('processors.tts must use runner "uv"');
  const projectDir = resolve(PACKAGE_ROOT, "processors", tts.name);
  return {
...
export function synthesizePlanInvocation({
  config,
  planPath,
  outputDir,
  root,
...
export function synthesizeCapabilities(start = process.cwd()): TtsCapabilities {
  ...
  if (!config.processors.tts)
    throw new Error("processors.tts is not configured in yumoframe.config.json");
...
async function forcedAlign(
  ...
  const outputBase = resolve(
    dirname(audioPath),
    `yf-tmp-align-${process.pid}-${basename(audioPath, extname(audioPath))}`,
  );
  writeFileSync(textPath, text);
  try {
    if (align.runner === "command") {
      if (align.command.length === 0)
        throw new Error("processors.align.command must be a non-empty array");
      await run(align.command[0]!, [...align.command.slice(1), audioPath, textPath, outputBase], {
        ...process.env,
        ...align.env,
      });
    } else if (align.runner === "uv") {
      const projectDir = resolve(PACKAGE_ROOT, "processors", align.name);
      ...
      await run(
        ...
        ],
        {
          ...process.env,
          ...align.env,
          UV_PROJECT_ENVIRONMENT: processorEnvironmentDir(align.name, config.runtimeVersion),
        },
      );
    } else {
...
async function alignFragments(
  config: YumoFrameConfig,
  align: Processor,
  plan: SpeechPlan,
  fragments: string[],
  directory: string,
): Promise<FragmentAlignment> {
  ...
  try {
    if (align.runner === "command") {
      ...
      await run(
        align.command[0]!,
        [...align.command.slice(1), "--align-manifest", manifestPath, "-o", outputBase],
        { ...process.env, ...align.env },
      );
    } else if (align.runner === "uv") {
      const projectDir = resolve(PACKAGE_ROOT, "processors", align.name);
...
function mergeFragmentTranscripts(
  ...
  if (
    alignment?.version !== 1 ||
    !Array.isArray(alignment.items) ||
    alignment.items.length !== plan.segments.length
  )
    return undefined;
  let offset = 0;
  const segments = [];
  for (const [index, planned] of plan.segments.entries()) {
    ...
    if (
      ...
      item.id !== planned.id ||
      !item.valid ||
      !Number.isFinite(fragmentDuration) ||
      fragmentDuration < 0
    )
      return undefined;
    if (alignmentIssues(item.transcript, planned.text, fragmentDuration).length > 0)
      return undefined;
    ...
    segments.push(
      ...item.transcript.segments.map((segment) => ({
        ...
        ...(segment.timestamp
          ? {
              timestamp: segment.timestamp.map(([start, end, ...rest]) => [
                start + offsetMs,
                end + offsetMs,
                ...rest,
              ]),
...
function alignmentIssues(
  ...
  const expectedTokens = (text.match(/[A-Za-z0-9]+|./gsu) ?? []).filter(
    (piece) => !UNSPOKEN_PUNCTUATION.has(piece) && !/^\s+$/u.test(piece),
  ).length;
  if (
    !transcript ||
      ...
      (segment) =>
        ...
        segment.timestamp.some(
          (stamp) =>
            !Array.isArray(stamp) ||
            stamp.length < 2 ||
            !Number.isFinite(stamp[0]) ||
            !Number.isFinite(stamp[1]),
        ),
    )
  ) {
  ...
  if (
    timestamps.some(
      ([start, end]) =>
        start === undefined || end === undefined || start < 0 || end > duration * 1000,
    )
  ) {
    issues.push("timestamp-out-of-range");
  }
  if (duration - timestamps.at(-1)![1]! / 1000 > MAX_TRAILING_AUDIO_SECONDS)
    issues.push("trailing-audio");
  return issues;
}

async function fallbackToAsr({
  root,
  config,
  output,
  outputPath,
  ...
  try {
    ...
    const transcript = parseTranscript(
      readFileSync(`${outputBase}.json`, "utf8"),
      `${outputBase}.json`,
    );
    ...
    console.warn(
      `ASR fallback unavailable (${error instanceof Error ? error.message : error}); keeping audio only.`,
    );
...
async function planFragments({
  ...
  if (tts.runner === "uv" && profile.execution === "native-batch") {
    if (tts.name !== "qwen3-tts")
      throw new Error(`TTS profile '${profile.id}' has no bundled plan-aware worker`);
    const invocation = synthesizePlanInvocation({ config, planPath, outputDir, root });
    await run(invocation.command, invocation.args, invocation.env);
    const outputs = plan.segments.map((_, index) =>
      resolve(outputDir, `${String(index).padStart(4, "0")}.wav`),
    );
    if (outputs.some((path) => !existsSync(path)))
      throw new Error(`TTS profile '${profile.id}' did not write every planned fragment`);
    return outputs;
  }
  if (tts.runner === "uv" && plan.segments.length === 1) {
    const output = resolve(outputDir, `0000${extension}`);
    const invocation = synthesizeInvocation({
      config,
  ...
  if (tts.runner === "uv")
    throw new Error(
      `TTS profile '${profile.id}' cannot execute a segmented plan without a plan-aware worker`,
    );
  ...
  for (const [index, segment] of plan.segments.entries()) {
    ...
    if (tts.runner === "api") {
      const options =
        segment.control.type === "openai-speech"
...
async function mergeFragments(
  ...
  if (fragments.length !== plan.segments.length)
    throw new Error("TTS fragment count does not match speech plan");
  const filter: string[] = [];
  const inputs: string[] = [];
  for (const [index, fragment] of fragments.entries()) {
    inputs.push("-i", fragment);
    filter.push(
      `[${index}:a]aformat=sample_fmts=fltp:sample_rates=24000:channel_layouts=mono[a${index}]`,
    );
  }
  const concat: string[] = [];
  for (const [index, segment] of plan.segments.entries()) {
    ...
    if (segment.pauseAfterMs > 0) {
      filter.push(
        `anullsrc=r=24000:cl=mono:d=${(segment.pauseAfterMs / 1000).toFixed(3)}[s${index}]`,
      );
      concat.push(`[s${index}]`);
    }
  }
...
export async function synthesizeProject({
  ...
  if (requestedPlan && (text !== undefined || input !== undefined))
    throw new Error("--plan cannot be combined with --text or --input");
  const planPath = requestedPlan ? resolve(root, requestedPlan) : undefined;
  const speechPlan = planPath
    ? parseSpeechPlan(readFileSync(planPath, "utf8"), requestedPlan)
    : undefined;
  ...
  if (speechPlan && resolve(root, speechPlan.source) !== canonicalTextPath) {
    throw new Error(
      `speech.json source must reference the configured TTS text: ${config.paths.ttsText ?? "text.txt"}`,
    );
  }
  const content = speechPlan
    ? readFileSync(canonicalTextPath, "utf8")
    : (text ??
      readFileSync(resolve(root, input ?? config.paths.ttsText ?? "text.txt"), "utf8").trim());
  ...
  try {
    ...
    if (speechPlan && planPath) {
      if (tts.runner === "uv" && tts.name === "qwen3-tts" && extension !== ".wav")
        throw new Error("Qwen3-TTS speech plans require a .wav output path");
      mkdirSync(segmentsDir, { recursive: true });
      const fragments = await planFragments({
        config,
        root,
        planPath,
        plan: speechPlan,
      ...
      if (config.processors.align) {
        try {
          fragmentAlignment = await alignFragments(
            config,
            config.processors.align,
            speechPlan,
            fragments,
            segmentsDir,
          );
          ...
          console.warn(
            `Fragment alignment failed (${error instanceof Error ? error.message : error}); falling back after audio merge.`,
          );
        }
      }
      ...
      if (tts.command.length === 0)
        throw new Error("processors.tts.command must be a non-empty array");
      ...
      if (!existsSync(resolve(projectDir, "pyproject.toml")))
        throw new Error(
          `Local TTS engine '${tts.name}' is not bundled yet; use runner "api" or "command"`,
        );
    ...
    if (speechPlan) {
      const transcript = fragmentAlignment
        ? mergeFragmentTranscripts(fragmentAlignment, speechPlan, duration)
        : undefined;
    ...
    if (subtitlesProduced && existsSync(subsPath) && statSync(subsPath).size > 0) {
      try {
        ...
        console.warn(
          `Subtitle parse failed (${error instanceof Error ? error.message : error}); falling back.`,
        );
      }
    }
    ...
    if (config.processors.align) {
      try {
        ...
        console.warn(
          `Forced alignment failed (${error instanceof Error ? error.message : error}); falling back.`,
        );
      }
    }
