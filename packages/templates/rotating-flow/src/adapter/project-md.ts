 ...
 */
export function formatProjectMd(
  ...
  for (const scene of project.timeline?.scenes ?? []) {
    ...
    lines.push(
      `## ${scene.id}  ${formatClock(scene.start)}–${formatClock(scene.end)}  rotate:${rotate}`,
    );
...
export function parseProjectMd(markdown: string): ParsedProjectMd {
  ...
  if (scenes.some((scene) => scene.lines.length === 0))
    throw new Error("project.md has an empty scene");
  // Mirrors MAX_LINES_PER_SCENE in rotating-flow.
  if (scenes.some((scene) => scene.lines.length > 4))
    throw new Error("project.md scene exceeds 4 lines");
 ...
 */
export function storyboardFromProject(project: RotatingFlowProject): Storyboard {
  const scenes: StoryboardScene[] = [];
  for (const scene of project?.timeline?.scenes ?? []) {
    // rotating-flow resolves one kinetic-text element per scene.
  ...
  return {
    ...
    template: project?.template || "rotating-flow",
