 ...
 */
export function getSceneFitFraming(
...
function getPivotAdjustedTarget(
  camera: Camera,
  pivotX: number,
  pivotY: number,
  width: number,
...
export function getCamera(
  ...
  if (!scene) {
    return {
      targetX: project.composition.width / 2,
      targetY: project.composition.height / 2,
      scale: 1,
      rotate: 0,
      ease: "linear",
    };
  ...
  const nextFit = next
    ? getSceneFitFraming(project, next, next.elements[0]?.lines[0]?.start ?? next.start)
    : currentFit;
    ...
    const target = getTargetForScreenPoint(
      overview.center,
      {
        x: interpolate(
          zoomProgress,
          [0, 1],
          [overviewCenterStart.x, project.composition.width / 2],
        ),
        y: interpolate(
          zoomProgress,
          [0, 1],
          [overviewCenterStart.y, project.composition.height / 2],
        ),
      },
      scale,
      scene.camera.rotate,
  ...
  const targetX =
    inTransition && next
      ? interpolate(progress, [0, 1], [currentTarget.targetX, nextTarget.targetX])
      : currentTarget.targetX;
  const targetY =
    inTransition && next
      ? interpolate(progress, [0, 1], [currentTarget.targetY, nextTarget.targetY])
      : currentTarget.targetY;
  const scale =
    inTransition && next
      ? interpolate(progress, [0, 1], [currentFit.scale, nextFit.scale])
      : currentFit.scale;
