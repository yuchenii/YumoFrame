...
import { alignStoryboardLines, buildCharTimeline } from "../packages/cli/dist/media/align.js";
import {
  formatProjectMd,
  markdownToSegments,
  parseProjectMd,
...
import {
  ...
  formatTranscriptMd,
  parseTranscriptMd,
} from "../packages/cli/dist/media/transcript-md.js";
...
import { syncProject } from "../packages/cli/dist/commands/sync.js";
...
`;
  ...
  assert.deepEqual(
    parsed.scenes.map((scene) => scene.lines.length),
    [2, 2],
  );
  assert.equal(
    parsed.scenes[0].lines[0].segments.find((segment) => segment.highlight).text,
    "复读",
  );
});
...
test("text-only project.md sync preserves existing line times", () => {
  const root = initProject({
    dir: join(mkdtempSync(join(tmpdir(), "yumoframe-project-md-")), "project"),
...
test("resolve uses nested scenes", () => {
  ...
  const project = resolveRotatingFlow(storyboard, { render: {} });
...
test("skill line-units script matches rotating-flow lineUnits", async () => {
  ...
  const { lineUnits, stripPunctuation } =
    await import("../packages/templates/rotating-flow/adapter-dist/index.js");
  const script = join(
    dirname(fileURLToPath(import.meta.url)),
    "../packages/templates/rotating-flow/scripts/line-units.mjs",
  );
...
test("universal Skill delegates template rules to inspect", () => {
  const skillRoot = join(dirname(fileURLToPath(import.meta.url)), "../skills/yumoframe");
  ...
  assert.match(skill, /^name: yumoframe$/m);
