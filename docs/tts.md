...
| `api` | 线上 TTS（阿里云原生 Qwen-TTS / CosyVoice、OpenAI 兼容接口等） | 只设一个 API key，连 uv 都不用 |
...
DashScope 默认使用仍可用的北京地域公共 API 根路径 `https://dashscope.aliyuncs.com/api/v1`。如需使用阿里云推荐的业务空间专属域名，可通过 `baseUrl` 配置 `https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api/v1`；CosyVoice HTTP 当前仅在北京地域可用。当前 Qwen `options` 读取 `languageType`、`instructions` 和 `optimizeInstructions`：整段合成可为 instruct 模型设置全局 `instructions`；分段合成由 `speech.json` 逐段提供指令；布尔值 `optimizeInstructions` 保持为全局设置，并且请求中必须有全局或分段 `instructions`。`languageType` 可选 `Auto`、`Chinese`、`English`、`German`、`Italian`、`Portuguese`、`Spanish`、`Japanese`、`Korean`、`French`、`Russian`。

CosyVoice 3.5 使用同一个 `api` runner，但走其非流式 HTTP 协议：
...
"tts": {
  ...
  "voice": "<创建时 target_model 为 cosyvoice-v3.5-plus 的音色 ID>",
  "apiKeyEnv": "DASHSCOPE_API_KEY",
  "options": {
    "instructions": "自然、有节奏的中文脱口秀表达，包袱前稍作停顿",
    "languageHints": ["zh"]
  }
}
```

这两个内置 CosyVoice 模型共用 `dashscope-cosyvoice-instruct` profile，调用 [`/services/audio/tts/SpeechSynthesizer`](https://help.aliyun.com/zh/model-studio/cosyvoice-tts-http-api)，支持整段或按计划顺序合成。YumoFrame 的该 profile 要求提供 `instructions`，以免配置了表演模型却静默退化为无指令合成。CosyVoice 3.5 不支持系统音色；当前只接入已有的复刻/设计音色 ID，不创建、查询或删除音色，也不接 WebSocket/SSE。音色创建时的 `target_model` 必须与这里的 `model` 完全一致，Plus 和 Flash 的音色不能混用。输出扩展名必须是 `.wav`、`.mp3` 或 `.opus`；`languageHints` 如需设置只能传一个受支持的语言代码。

内置模型的 `protocol` 和 `profile` 由统一注册表确定，正常配置不需要重复填写；显式填写但与注册表冲突会在请求前报错。自定义 API 模型没有可安全推断的请求格式，因此必须同时声明已知的 `protocol` 与 `profile`，例如 OpenAI 兼容服务使用 `"protocol": "openai-compatible"` 和 `"profile": "api-single-neutral"`。`baseUrl` 只替换服务地域或兼容端点，不改变协议。
...
| DashScope `cosyvoice-v3.5-plus`      | `dashscope-instruct` | 顺序调用非流式 HTTP      |
| DashScope `cosyvoice-v3.5-flash`     | `dashscope-instruct` | 顺序调用非流式 HTTP      |
