...
import {synthesizeCapabilities, synthesizeInvocation, synthesizePlanInvocation, synthesizeProject} from '../dist/commands/synthesize.js';
import {apiSpeechBody, dashscopeSpeechBody, processorEnvironmentDir, runApiSpeech} from '../dist/processor-runtime.js';
...
import {renderLayoutSvg} from '../dist/commands/layout.js';
...
test('layout SVG fits its viewBox to positioned content instead of the virtual canvas', () => {
  const project = {
    ...
    timeline: {
      ...
      scenes: [{
        id: 'scene-001', start: 0, end: 1,
        camera: {targetX: 0, targetY: 0, scale: 1, rotate: 0, ease: 'spring'},
        elements: [{
          id: 'text-001', type: 'kinetic-text', x: 0, y: 0, width: 0, rotate: 0, scale: 1,
          fontSize: 100, lineHeight: 1, align: 'right',
  ...
  assert.match(svg, /width="502" height="500" viewBox="19749 1650 502 500"/);
});
...
test('end overview centers and contains all content', async () => {
  ...
  const templateRoot = join(dirname(fileURLToPath(import.meta.url)), '../runtime/templates/comedy-text');
  const server = await createServer({root: templateRoot, logLevel: 'silent', server: {middlewareMode: true}, appType: 'custom'});
  try {
    const {getCamera} = await server.ssrLoadModule('/src/lib/camera.ts');
    const line = (text, start, end) => ({fontSize: 100, start, end, segments: [{text, highlight: false}]});
    const element = (id, x, y, text, start, end) => ({
      id, type: 'kinetic-text', x, y, width: 200, rotate: 0, scale: 1, fontSize: 100, lineHeight: 1,
      align: 'left', lines: [line(text, start, end)],
    });
    const project = {
      version: '0.1.0', template: 'comedy-text', endOverview: true,
      composition: {width: 1000, height: 1000, fps: 30, duration: 4, background: '#000'},
      source: {type: 'text', text: '甲乙'},
      theme: {fontFamily: 'system-ui', textColor: '#fff', highlightColor: '#0f0', cursorColor: '#fff', dimCursorColor: '#777'},
      timeline: {virtualCanvas: {width: 40000, height: 40000}, scenes: [
        {id: 's1', start: 0, end: 1, camera: {targetX: 200, targetY: 250, scale: 1, rotate: 0, ease: 'spring'}, elements: [element('e1', 100, 200, '甲', 0, 1)]},
        {id: 's2', start: 1, end: 2, camera: {targetX: 1000, targetY: 650, scale: 1, rotate: 0, ease: 'spring'}, elements: [element('e2', 900, 600, '乙', 1, 2)]},
      ]},
    };
    ...
    const zoomProgress = (overviewStartCamera.scale - earlyOverviewCamera.scale) / (overviewStartCamera.scale - camera.scale);
    ...
    assert.ok(Math.abs(moved / totalMove - zoomProgress) < 0.001, 'overview should center in sync with its zoom');
  } finally {
    await server.close();
...
test('Qwen3-TTS uses the bundled uv processor with 0.6B Vivian defaults', () => {
  ...
  assert.equal(invocation.command, 'uv');
  assert.equal(invocation.env.UV_PROJECT_ENVIRONMENT, processorEnvironmentDir('qwen3-tts', '0.1.1'));
  assert.ok(invocation.args.includes('--locked'));
  assert.ok(invocation.args.some((arg) => arg.endsWith('/runtime/processors/qwen3-tts')));
  assert.deepEqual(invocation.args.slice(4, 9), ['qwen3-tts', '--text', '大家好', '--output', '/tmp/voice.wav']);
  ...
  const cloneInvocation = synthesizeInvocation({config, text: '大家好', outPath: '/tmp/voice.wav', root: '/tmp/project'});
  assert.ok(cloneInvocation.args.includes('/tmp/project/assets/reference.wav'));

  const planInvocation = synthesizePlanInvocation({config, planPath: '/tmp/project/speech.json', outputDir: '/tmp/project/parts', root: '/tmp/project'});
  assert.deepEqual(planInvocation.args.slice(4, 9), ['qwen3-tts', '--plan', '/tmp/project/speech.json', '--output-dir', '/tmp/project/parts']);
});

test('VoiceDesign whole-text requirements fail before starting uv', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-voice-design-')), 'project'), template: 'comedy-text'});
...
test('speech plans validate against the active profile before merging and alignment', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-speech-plan-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  const fakeAlign = join(root, 'fake-align.mjs');
  const calls = join(root, 'tts-calls.txt');
  writeFileSync(fakeTts, `
...
`);
  const configPath = join(root, 'yumoframe.config.json');
  const config = json(configPath);
  config.processors.tts = {runner: 'command', profile: 'edge-tts', command: [process.execPath, fakeTts, '{out}', calls]};
  ...
  const plan = {
    version: '0.1.0', source: 'text.txt', segments: [
      {id: 's1', text: '第一句。', intent: {emotion: 'calm', intensity: 0.2, pace: 'normal'}, control: {type: 'qwen-instruct', instruct: '平静'}, pauseAfterMs: 100},
      {id: 's2', text: '第二句！', intent: {emotion: 'surprised', intensity: 0.8, pace: 'fast'}, control: {type: 'qwen-instruct', instruct: '惊讶'}, pauseAfterMs: 0},
    ],
  };
  ...
  writeFileSync(planPath, `${JSON.stringify(plan, null, 2)}\n`);
  await assert.rejects(synthesizeProject({plan: 'speech.json', start: root}), /source must reference the configured TTS text/);
  assert.equal(existsSync(calls), false);
  plan.source = 'text.txt';
  writeFileSync(planPath, `${JSON.stringify(plan, null, 2)}\n`);
  await assert.rejects(synthesizeProject({plan: 'speech.json', start: root}), /uses qwen-instruct.*accepts edge-prosody/);
  assert.equal(existsSync(calls), false);
  ...
  assert.equal(json(result.transcriptPath).segments.map((segment) => segment.text).join(''), '第一句。第二句！');
});

test('fragment alignment offsets transcripts by fragment duration and planned pauses', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-fragment-timing-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  const fakeAlign = join(root, 'fake-align.mjs');
  writeFileSync(fakeTts, `
...
`);
  const configPath = join(root, 'yumoframe.config.json');
  const config = json(configPath);
  config.processors.tts = {runner: 'command', profile: 'edge-tts', command: [process.execPath, fakeTts, '{out}', '{text}']};
  config.processors.align = {runner: 'command', command: [process.execPath, fakeAlign]};
  config.processors.asr = {runner: 'command', command: [join(root, 'must-not-run')]};
  writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);
  writeFileSync(join(root, 'text.txt'), '甲乙');
  writeFileSync(join(root, 'speech.json'), `${JSON.stringify({
    version: '0.1.0', source: 'text.txt', segments: [
      {id: 's1', text: '甲', intent: {emotion: 'neutral', intensity: 0, pace: 'normal'}, control: {type: 'edge-prosody', rate: '+0%', pitch: '+0Hz', volume: '+0%'}, pauseAfterMs: 250},
      {id: 's2', text: '乙', intent: {emotion: 'neutral', intensity: 0, pace: 'normal'}, control: {type: 'edge-prosody', rate: '+0%', pitch: '+0Hz', volume: '+0%'}, pauseAfterMs: 0},
    ],
  }, null, 2)}\n`);
  ...
  await withFakeMediaTools(root, async () => {
    ...
    assert.deepEqual(transcript.segments.map(({start, end, timestamp}) => [start, end, timestamp]), [
      [0, 0.8, [[0, 800]]],
      [1.35, 3.15, [[1350, 3150]]],
    ]);
...
test('one invalid fragment falls back once to final-audio ASR without mixing timings', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-fragment-fallback-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  const fakeAlign = join(root, 'fake-align.mjs');
  const fakeAsr = join(root, 'fake-asr.mjs');
  const asrCalls = join(root, 'asr-calls.jsonl');
  writeFileSync(fakeTts, `import {writeFileSync} from 'node:fs'; writeFileSync(process.argv[2], JSON.stringify({duration: 1}));\n`);
  writeFileSync(fakeAlign, `
import {readFileSync, writeFileSync} from 'node:fs';
const manifest = JSON.parse(readFileSync(process.argv[process.argv.indexOf('--align-manifest') + 1], 'utf8'));
const output = process.argv[process.argv.indexOf('-o') + 1];
...
`);
  const configPath = join(root, 'yumoframe.config.json');
  const config = json(configPath);
  config.processors.tts = {runner: 'command', profile: 'edge-tts', command: [process.execPath, fakeTts, '{out}']};
  config.processors.align = {runner: 'command', command: [process.execPath, fakeAlign]};
  ...
  writeFileSync(join(root, 'speech.json'), `${JSON.stringify({
    version: '0.1.0', source: 'text.txt', segments: [
      {id: 's1', text: '甲', intent: {emotion: 'neutral', intensity: 0, pace: 'normal'}, control: {type: 'edge-prosody', rate: '+0%', pitch: '+0Hz', volume: '+0%'}, pauseAfterMs: 100},
      {id: 's2', text: '乙', intent: {emotion: 'neutral', intensity: 0, pace: 'normal'}, control: {type: 'edge-prosody', rate: '+0%', pitch: '+0Hz', volume: '+0%'}, pauseAfterMs: 0},
    ],
  }, null, 2)}\n`);
  ...
  await withFakeMediaTools(root, async () => {
    ...
    assert.deepEqual(json(result.transcriptPath).segments.map((segment) => segment.text), ['实际朗读']);
...
test('implausible whole-audio alignment falls back to ASR', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-whole-fallback-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  const fakeAlign = join(root, 'fake-align.mjs');
  const fakeAsr = join(root, 'fake-asr.mjs');
  const asrCalls = join(root, 'asr-calls.txt');
  writeFileSync(fakeTts, `import {writeFileSync} from 'node:fs'; writeFileSync(process.argv[2], JSON.stringify({duration: 10}));\n`);
  writeFileSync(fakeAlign, `
import {writeFileSync} from 'node:fs';
...
test('synthesize keeps audio-only output when ASR is unavailable', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-audio-only-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  writeFileSync(fakeTts, `import {writeFileSync} from 'node:fs'; writeFileSync(process.argv[2], JSON.stringify({duration: 2.5}));\n`);
  const configPath = join(root, 'yumoframe.config.json');
  const config = json(configPath);
  config.processors.tts = {runner: 'command', command: [process.execPath, fakeTts, '{out}']};
...
test('synthesize CLI emits a stable timing summary without processor secrets', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-summary-')), 'project'), template: 'comedy-text'});
...
`);
  ...
  const config = json(configPath);
  config.processors.tts = {
    runner: 'uv', name: 'qwen3-tts', uvBin: fakeUvShim,
    env: {TTS_SECRET: 'must-not-leak'},
    options: {model: 'Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice', language: 'Chinese', speaker: 'Vivian', device: 'cpu'},
  };
  config.processors.align = {runner: 'command', command: [process.execPath, fakeAlign]};
  writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);

  await withFakeMediaTools(root, async () => {
    const cli = spawnSync(process.execPath, [cliPath, 'synthesize', '--text', '你好'], {cwd: root, encoding: 'utf8'});
    assert.equal(cli.status, 0, cli.stderr);
    const summary = JSON.parse(cli.stdout);
    assert.deepEqual(Object.keys(summary), ['outputPath', 'transcriptPath', 'duration', 'tts', 'timingMode', 'lastTimestamp', 'coverage', 'reviewRequired']);
    assert.deepEqual(summary.tts, {
      runner: 'uv', processor: 'qwen3-tts', provider: null,
      model: 'Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice', voice: 'Vivian', profile: 'qwen3-custom-voice',
    });
...
test('speech plan parser enforces source fidelity and model-specific controls', () => {
  const raw = JSON.stringify({version: '0.1.0', source: 'text.txt', segments: [{
    id: 's1', text: '你好', intent: {emotion: 'neutral', intensity: 0, pace: 'normal'},
    control: {type: 'none'}, pauseAfterMs: 0,
  }]});
  const plan = parseSpeechPlan(raw);
  const baseProcessor = {runner: 'uv', name: 'qwen3-tts', options: {model: 'Qwen/Qwen3-TTS-12Hz-1.7B-Base', refAudio: 'voice.wav'}};
  ...
  assert.throws(
    () => resolveTtsProfile({runner: 'command', command: ['edge-tts'], profile: 'qwen3-custom-voice'}),
    /not compatible/,
  );
  assert.throws(
    () => resolveTtsProfile({
      runner: 'uv', name: 'qwen3-tts', profile: 'qwen3-custom-voice',
  ...
  assert.throws(
    () => validateTtsConfiguration(base, {...baseProcessor, options: {model: baseProcessor.options.model}}),
    /requires non-empty processors\.tts\.options\.refAudio/,
  );
  assert.throws(() => validateTtsConfiguration(base, baseProcessor, tmpdir()), /cannot find.*voice\.wav/);
  assert.throws(() => parseSpeechPlan(raw.replace('"pauseAfterMs":0', '"pauseAfterMs":10001')), /between 0 and 10000/);
  assert.throws(() => parseSpeechPlan(raw.replace('"type":"none"', '"type":"none","instruct":"ignored"')), /unsupported field: instruct/);
});
...
test('speech splitter preserves every source character across mechanical units', () => {
  ...
  const script = join(dirname(fileURLToPath(import.meta.url)), '../runtime/skills/yumoframe-comedy-text/scripts/split-speech.mjs');
  ...
  assert.deepEqual(split.units.map((unit) => unit.id), ['u1', 'u2', 'u3']);
});
...
test('OpenAI speech profile exposes only supported delivery fields and protects canonical request fields', () => {
  ...
  const plan = parseSpeechPlan(JSON.stringify({version: '0.1.0', source: 'text.txt', segments: [{
    id: 's1', text: 'Hello', intent: {emotion: 'warm', intensity: 0.5, pace: 'normal'},
    control: {type: 'openai-speech', instructions: 'Speak warmly.', speed: 1.1}, pauseAfterMs: 0,
  }]}));
  assert.doesNotThrow(() => validateSpeechPlan(plan, 'Hello', profile));
  assert.throws(
    () => parseSpeechPlan(JSON.stringify({...plan, segments: [{...plan.segments[0], control: {type: 'openai-speech', instructions: 'Fast.', speed: 4.1}}]})),
    /between 0.25 and 4/,
  );
  assert.deepEqual(
    apiSpeechBody(
      {...processor, options: {model: 'wrong-static', input: 'wrong-static'}},
      {text: 'Authoritative text', options: {model: 'wrong-segment', input: 'wrong-segment', instructions: 'Warm.'}},
    ),
    {model: 'gpt-4o-mini-tts', input: 'Authoritative text', instructions: 'Warm.', voice: 'alloy'},
  );
});

test('DashScope uses its native Qwen TTS request and downloads returned audio', async () => {
  const processor = {
    runner: 'api', provider: 'dashscope', model: 'qwen3-tts-instruct-flash', voice: 'Cherry', apiKeyEnv: 'YF_TEST_DASHSCOPE_KEY',
    options: {languageType: 'Chinese', optimizeInstructions: true},
  };
  const profile = resolveTtsProfile(processor);
  assert.equal(profile.id, 'dashscope-qwen3-tts-instruct-flash');
  const plan = parseSpeechPlan(JSON.stringify({version: '0.1.0', source: 'text.txt', segments: [{
    id: 's1', text: '你好', intent: {emotion: 'happy', intensity: 0.6, pace: 'normal'},
    control: {type: 'dashscope-instruct', instructions: '开心自然'}, pauseAfterMs: 0,
  }]}));
  assert.doesNotThrow(() => validateSpeechPlan(plan, '你好', profile));
  assert.equal(resolveTtsProfile({...processor, provider: 'qwen3-tts'}).id, profile.id);
  assert.equal(resolveTtsProfile({...processor, model: 'qwen3-tts-flash'}).id, 'dashscope-qwen3-tts-flash');
  assert.deepEqual(
    dashscopeSpeechBody(processor, {text: '你好', options: {instructions: '语速稍快，结尾上扬'}}),
    {model: processor.model, input: {
      text: '你好', voice: 'Cherry', language_type: 'Chinese',
      instructions: '语速稍快，结尾上扬', optimize_instructions: true,
    }},
  );
  assert.throws(
    () => dashscopeSpeechBody({...processor, model: 'qwen3-tts-flash'}, {text: '你好', options: {instructions: '开心'}}),
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (requests.length === 1) {
      return new Response(JSON.stringify({output: {audio: {url: 'https://audio.example/speech.wav'}}}), {
  ...
  try {
    await runApiSpeech(processor, {text: '你好', outPath: output, options: {instructions: '开心自然'}});
    globalThis.fetch = async () => new Response(JSON.stringify({output: {audio: {data: '%%%'}}}), {
      status: 200, headers: {'Content-Type': 'application/json'},
    });
    await assert.rejects(
      runApiSpeech(processor, {text: '你好', outPath: output, options: {instructions: '开心自然'}}),
      /invalid Base64 audio/,
    );
    let emptyDownloadRequest = 0;
    globalThis.fetch = async () => emptyDownloadRequest++ === 0
      ? new Response(JSON.stringify({output: {audio: {url: 'https://audio.example/empty.wav'}}}), {
          status: 200, headers: {'Content-Type': 'application/json'},
        })
...
test('forced alignment reads the suffix-replaced FunASR output', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-align-output-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  const fakeAlign = join(root, 'fake-align.mjs');
  writeFileSync(fakeTts, `import {writeFileSync} from 'node:fs'; writeFileSync(process.argv[2], JSON.stringify({duration: 1}));\n`);
  writeFileSync(fakeAlign, `
...
`);
  ...
  await withFakeMediaTools(root, async () => {
    ...
    assert.deepEqual(json(result.transcriptPath).segments.map((segment) => segment.text), ['你好']);
  });
});

test('synthesize persists a project-local output as the narration path', async () => {
  const root = initProject({dir: join(mkdtempSync(join(tmpdir(), 'yumoframe-tts-output-')), 'project'), template: 'comedy-text'});
  const fakeTts = join(root, 'fake-tts.mjs');
  writeFileSync(fakeTts, `import {writeFileSync} from 'node:fs'; writeFileSync(process.argv[2], JSON.stringify({duration: 1}));\n`);
  const configPath = join(root, 'yumoframe.config.json');
  const config = json(configPath);
  ...
  await withFakeMediaTools(root, async () => {
    ...
    assert.equal(resolveProject(root, {align: false}).project.audio.voice.src, 'assets/custom.mp3');

    await synthesizeProject({text: '你好', out: 'assets/second.mp3', start: root});
    assert.equal(resolveProject(root, {align: false}).project.audio.voice.src, 'assets/second.mp3');

    const outside = join(dirname(root), 'outside.mp3');
    await assert.rejects(synthesizeProject({text: '你好', out: outside, start: root}), /inside the project/);
    assert.equal(existsSync(outside), false);

    const escaped = join(dirname(root), 'escaped');
    mkdirSync(escaped);
    symlinkSync(escaped, join(root, 'linked'), process.platform === 'win32' ? 'junction' : 'dir');
    await assert.rejects(synthesizeProject({text: '你好', out: 'linked/escaped.mp3', start: root}), /inside the project/);
...
test('doctor and template listing expose required runtime capabilities', () => {
  ...
  assert.deepEqual(doctorChecks(() => '/mock/bin/tool').map((check) => check.name), ['node', 'uv', 'ffmpeg', 'comedy-text', 'funasr', 'qwen3-tts']);
