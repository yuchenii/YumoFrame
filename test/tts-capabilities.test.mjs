...
import {synthesizeCapabilities} from '../dist/commands/synthesize.js';

const json = (path) => JSON.parse(readFileSync(path, 'utf8'));
const projectWithTts = (tts) => {
  const root = mkdtempSync(join(tmpdir(), 'yumoframe-tts-capabilities-'));
  writeFileSync(join(root, 'yumoframe.config.json'), `${JSON.stringify({
    version: '0.1.0',
    ...
    templatePath: null,
    paths: {
      media: 'assets/input.wav', voice: 'assets/input.wav', transcript: 'transcript.json', transcriptMd: 'transcript.md',
      lines: 'lines.json', storyboard: 'storyboard.json', project: 'project.json', projectMd: 'project.md',
      layoutSvg: 'out/layout-preview.svg', assets: 'assets', output: 'out/video.mp4', ttsText: 'text.txt',
    },
...
test('default Qwen3-TTS capabilities report the selected configuration and local catalog', () => {
  ...
  assert.deepEqual(capabilities.available.models.map(({model, profile}) => ({model, profile})), [
  ...
  assert.deepEqual(capabilities.available.voices.map(({speaker, nativeLanguage}) => ({speaker, nativeLanguage})), [
...
test('1.7B CustomVoice capabilities keep the configured model and speaker selected', () => {
  ...
  assert.ok(capabilities.available.models.some(({model}) => model === capabilities.selected.model));
  assert.ok(capabilities.available.voices.some(({speaker}) => speaker === capabilities.selected.speaker));
});
...
test('model names with text after a known suffix remain unknown', () => {
  ...
  assert.deepEqual(capabilities.profile, {id: 'unknown', execution: 'single', controls: ['none'], timing: 'align'});
});
...
test('custom API capabilities expose no key, environment, endpoint, or private options', () => {
  ...
  try {
    ...
    for (const secret of ['YF_PRIVATE_KEY', 'environment-secret', 'inline-secret', 'secret.example', 'url-secret']) {
