...
import type {Processor, SpeechControl, SpeechPlan, SpeechPlanSegment, TtsCapabilities, TtsProfile} from './types.js';
...
type TtsRegistry = {profiles: RegisteredProfile[]; models: RegisteredModel[]; voices: RegisteredVoice[]};

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const registryPath = resolve(packageRoot, 'runtime', 'processors', 'tts-profiles.json');
...
function exactKeys(value: JsonRecord, label: string, allowed: string[]): void {
  ...
  if (unexpected.length > 0) throw new Error(`${label} has unsupported field${unexpected.length === 1 ? '' : 's'}: ${unexpected.join(', ')}`);
}
...
function numberIn(value: JsonRecord, key: string, label: string, min: number, max: number): number {
  const result = value[key];
  if (!finite(result) || result < min || result > max) throw new Error(`${label}.${key} must be between ${min} and ${max}`);
  return result;
}
...
function parseControl(value: unknown, label: string): SpeechControl {
  ...
  if (type === 'indextts2-emo-text') {
    ...
    return {type, emoText: string(control, 'emoText', label), emoAlpha: numberIn(control, 'emoAlpha', label, 0, 1)};
  }
  if (type === 'indextts2-emo-vector') {
    exactKeys(control, label, ['type', 'emoVector']);
    const vector = control.emoVector;
    if (!Array.isArray(vector) || vector.length !== 8 || !vector.every((item) => finite(item) && item >= 0 && item <= 0.8)) {
  ...
  if (type === 'indextts2-emo-audio') {
    ...
    return {type, emoAudio: string(control, 'emoAudio', label), emoAlpha: numberIn(control, 'emoAlpha', label, 0, 1)};
  }
  ...
  if (type === 'openai-speech') {
    ...
    return {type, instructions: string(control, 'instructions', label), speed: numberIn(control, 'speed', label, 0.25, 4)};
  }
...
export function parseSpeechPlan(text: string, label = 'speech.json'): SpeechPlan {
  ...
  if (!Array.isArray(value.segments) || value.segments.length === 0) throw new Error(`${label}.segments must be a non-empty array`);
  ...
  const segments: SpeechPlanSegment[] = value.segments.map((rawSegment, index) => {
    ...
    if (pace !== 'slow' && pace !== 'normal' && pace !== 'fast') throw new Error(`${segmentLabel}.intent.pace must be slow, normal, or fast`);
    if (intentValue.note !== undefined && typeof intentValue.note !== 'string') throw new Error(`${segmentLabel}.intent.note must be a string`);
    const pauseAfterMs = segment.pauseAfterMs;
    if (!Number.isInteger(pauseAfterMs) || !finite(pauseAfterMs) || pauseAfterMs < 0 || pauseAfterMs > 10000) {
      throw new Error(`${segmentLabel}.pauseAfterMs must be an integer between 0 and 10000`);
    }
    return {
      id,
      text: string(segment, 'text', segmentLabel),
      intent: {emotion, intensity, pace, ...(intentValue.note === undefined ? {} : {note: intentValue.note})},
...
function publicProfile(profile: RegisteredProfile): TtsProfile {
  return {
    ...
    ...(profile.requiredWholeTextConfig ? {requiredWholeTextConfig: profile.requiredWholeTextConfig} : {}),
...
function processorMatches(profile: RegisteredProfile, processor: Processor, checkModel: boolean): boolean {
  if (profile.compatibleRunners && !profile.compatibleRunners.includes(processor.runner)) return false;
  if (!profile.match) return true;
  if (profile.match.runner && profile.match.runner !== processor.runner) return false;
  if (processor.runner === 'uv' && profile.match.name && profile.match.name !== processor.name) return false;
  if (processor.runner === 'api' && profile.match.provider) {
    const providers = Array.isArray(profile.match.provider) ? profile.match.provider : [profile.match.provider];
    if (!providers.includes(processor.provider)) return false;
  }
  if (!checkModel || !profile.match.modelSuffix) return true;
  const model = processor.runner === 'uv' ? String(processor.options?.model ?? '') : processor.runner === 'api' ? processor.model ?? '' : '';
...
function resolveProfile(processor: Processor, registered: RegisteredProfile[]): TtsProfile {
  if (processor.profile) {
    ...
    if (!processorMatches(selected, processor, true)) {
      throw new Error(`TTS profile '${selected.id}' is not compatible with the configured ${processor.runner} processor`);
    }
    return publicProfile(selected);
  }
  const selected = registered.find((profile) => profile.match && processorMatches(profile, processor, true));
...
function catalogMatches(entry: RegisteredModel | RegisteredVoice, processor: Processor): boolean {
  ...
  if (processor.runner === 'api' && entry.provider) {
    return (Array.isArray(entry.provider) ? entry.provider : [entry.provider]).includes(processor.provider);
  }
  return false;
}
...
export function resolveTtsCapabilities(processor: Processor): TtsCapabilities {
  ...
  const model = processor.runner === 'uv'
    ? configuredString(options?.model)
    : processor.runner === 'api' ? configuredString(processor.model) : undefined;
  ...
  const selected: TtsCapabilities['selected'] = {
    ...
    ...(processor.runner === 'api' && configuredString(processor.voice) ? {voice: processor.voice} : {}),
    ...(device ? {device} : {}),
  };
  const models = registered.models.filter((entry) => catalogMatches(entry, processor)).map(({model, profile: modelProfile}) => ({model, profile: modelProfile}));
  const voices = registered.voices.filter((entry) => catalogMatches(entry, processor)).map(({speaker, description, nativeLanguage}) => ({speaker, description, nativeLanguage}));
  return {
    selected,
    ...(models.length > 0 ? {available: {models, ...(voices.length > 0 ? {voices} : {})}} : {}),
    profile,
  };
}

const neutralIntent = (segment: SpeechPlanSegment): boolean =>
  segment.intent.emotion === 'neutral' && segment.intent.intensity === 0 && segment.intent.pace === 'normal' && !segment.intent.note;
...
export function validateSpeechPlan(plan: SpeechPlan, sourceText: string, profile: TtsProfile): void {
  const joined = plan.segments.map((segment) => segment.text).join('');
  if (joined !== sourceText) throw new Error(`speech.json segments do not exactly reproduce ${plan.source}`);
  if (profile.execution === 'single' && plan.segments.length > 1) {
    throw new Error(`TTS profile '${profile.id}' supports only whole-text synthesis; use the direct CLI without --plan or choose a segmented profile`);
  }
  for (const segment of plan.segments) {
    if (!profile.controls.includes(segment.control.type)) {
      throw new Error(`segment ${segment.id} uses ${segment.control.type}, but TTS profile '${profile.id}' accepts ${profile.controls.join(', ')}`);
    }
    if (segment.control.type === 'none' && !neutralIntent(segment)) {
      throw new Error(`segment ${segment.id} has non-neutral intent, but TTS profile '${profile.id}' has no delivery control`);
    }
  }
  for (const path of profile.requiredPlan ?? []) {
    const value = valueAt(plan, path);
    if (typeof value !== 'string' || !value.trim()) throw new Error(`TTS profile '${profile.id}' requires non-empty speech.json ${path}`);
  }
}

/** Validate profile-specific global options and referenced local resources. */
export function validateTtsConfiguration(profile: TtsProfile, processor: Processor, root?: string, mode: 'whole' | 'plan' = 'whole'): void {
  const required = [...(profile.requiredConfig ?? []), ...(mode === 'whole' ? profile.requiredWholeTextConfig ?? [] : [])];
  ...
  for (const path of profile.configPaths ?? []) {
    ...
    if (root && typeof value === 'string' && !/^[a-z][a-z\d+.-]*:/i.test(value)) {
      ...
      if (!existsSync(absolute)) throw new Error(`TTS profile '${profile.id}' cannot find processors.tts.${path}: ${absolute}`);
    }
  }
}
