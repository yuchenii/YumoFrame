/**
 * Comedy-text template rules: line-unit limits, sanitization, validation, and project resolution.
 */
import {existsSync} from 'node:fs';
import {relative, resolve} from 'node:path';
import type {LinesDocument, Storyboard, TextLine, YumoFrameConfig, YumoFrameProject} from './types.js';
...
const PUNCT_RE = /[，。！？、；：""''“”‘’（）()\[\]【】《》〈〉…—\-~～,.!?;:'"`]/gu;
const hasPunctuation = (text: string) => /[，。！？、；：""''“”‘’（）()\[\]【】《》〈〉…—\-~～,.!?;:'"` ]/.test(text) || /\s/.test(text);
const DEFAULT_THEME = {
 ...
 * Keep in sync with skill scripts/line-units.js.
...
export function lineUnits(text: unknown): number {
  ...
  return [...String(text ?? '')].reduce((sum, char) => sum + (/[^\x00-\xff]/.test(char) ? 1 : 0.5), 0);
}
...
export function stripPunctuation(text: unknown): string {
  return String(text ?? '').replace(PUNCT_RE, '').replace(/\s+/g, '');
}
 ...
 */
function validateLineList(lines: TextLine[], errors: string[], labelFor: (index: number) => string): void {
  let previousEnd = 0;
  let highlighted = 0;
  let total = 0;
  let shortLines = 0;
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (typeof start === 'number' && typeof end === 'number') {
      ...
      if (start < previousEnd) {
        errors.push(`${label} starts at ${start} before previous line ended at ${previousEnd} — fix timing order, do not rewrite unrelated lines`);
      }
    ...
    for (const [segmentIndex, segment] of line.segments.entries()) {
      ...
      if (typeof segment.highlight !== 'boolean') errors.push(`${label}.segments[${segmentIndex}].highlight must be boolean`);
    ...
    if (!text) {
      errors.push(`${label} is empty after removing punctuation — delete the line or replace with real words`);
      continue;
    }
  ...
  if (total > 0 && highlighted / total > 0.35) {
    ...
    errors.push(`highlight ratio ${ratio}% exceeds 35% (${highlighted}/${total} chars) — un-highlight some tokens; prefer punch nouns/numbers only`);
  }
}
...
export function validateLinesDoc(doc: LinesDocument): string[] {
  const errors: string[] = [];
  if (doc?.template !== 'comedy-text') errors.push('template must be comedy-text');
...
export function validateStoryboard(storyboard: Storyboard): string[] {
  const errors: string[] = [];
  if (storyboard?.template !== 'comedy-text') errors.push('template must be comedy-text');
  if (!Array.isArray(storyboard?.scenes) || storyboard.scenes.length === 0) {
    errors.push('scenes must be a non-empty array — author storyboard.json as scenes[].lines (not flat lines)');
    return errors;
  }
 ...
 * @returns Resolved {@link YumoFrameProject}.
 */
export function resolveComedyText(storyboard: Storyboard, config: YumoFrameConfig): YumoFrameProject {
  ...
  const duration = Math.ceil((Math.max(lastEnd, sanitized.duration ?? lastEnd) + (endOverview ? OVERVIEW_PAD : 0.5)) * 10) / 10;
  ...
  const scenes = sanitized.scenes.map((scene, index) => {
    ...
    return {
      ...
      camera: {targetX: 0, targetY: 0, scale: 1, rotate: index % 2 === 0 ? 0 : -90, ease: 'spring'},
      elements: [{
        ...
        exit: 'fade-slide',
        lines: sceneLines.map((line) => ({segments: line.segments, start: round(line.start ?? 0), end: round(line.end ?? 0)})),
      }],
    };
  });

  const project: YumoFrameProject = {
    version: '0.1.0',
    ...
    // Large virtual canvas: layout places scenes far apart for camera pans.
    timeline: {virtualCanvas: {width: 40000, height: 40000}, scenes},
  };
  if (sanitized.audio?.src) project.audio = {voice: {src: sanitized.audio.src, start: 0, volume: 1, source: sanitized.audio.source ?? 'user'}};
  return project;
}

/**
 * Validate a resolved project.json against comedy-text rules and on-disk audio paths.
 * @param project - Resolved Remotion project payload.
 * @param projectRoot - Absolute project root for asset path checks.
 * @returns Human-readable error strings (empty when valid).
 */
export function validateProject(project: YumoFrameProject, projectRoot: string): string[] {
  const errors: string[] = [];
  if (project?.template !== 'comedy-text') errors.push('template must be comedy-text');
  ...
  for (const scene of project?.timeline?.scenes ?? []) {
    ...
    if (scene.start < previousSceneEnd) errors.push(`${scene.id} starts before previous scene ended`);
    if (![0, -90, 90].includes(scene.camera?.rotate)) errors.push(`${scene.id} camera.rotate must be 0, -90, or 90`);
    // Template invariant: each cut flips orientation by exactly 90°.
    if (previousRotate !== null && Math.abs(scene.camera.rotate - previousRotate) !== 90) errors.push(`${scene.id} must rotate 90 degrees from previous scene`);
  ...
  if (project?.timeline?.scenes?.length > 0 && !(project.composition.duration > lastLineEnd)) errors.push('composition.duration must be greater than the last line end');
  ...
  for (const [label, track] of tracks) {
    ...
    if (rel.startsWith('..') || rel === '') errors.push(`${label}.src must stay inside project root`);
...
export function nestAlignedLines(storyboard: Storyboard, alignedLines: TextLine[]): Storyboard {
  ...
  if (offset !== alignedLines.length) {
    throw new Error(`aligned line count ${alignedLines.length} does not match storyboard lines ${offset}`);
  }
  return {...storyboard, scenes};
}
