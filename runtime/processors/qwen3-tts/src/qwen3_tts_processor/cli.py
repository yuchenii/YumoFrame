...
def synthesize_plan(
    ...
    *,
    ...
    expected_control = {"custom_voice": "qwen-instruct", "voice_design": "qwen-voice-design", "voice_clone": "none"}[variant]
    for index, raw_segment in enumerate(segments):
        if not isinstance(raw_segment, dict) or not isinstance(raw_segment.get("text"), str) or not raw_segment["text"]:
    ...
    if variant == "custom_voice":
        ...
        wavs, sample_rate = model.generate_voice_design(text=texts, language=languages, instruct=instructions)
    else:
        ...
        wavs, sample_rate = model.generate_voice_clone(text=texts, language=languages, voice_clone_prompt=prompt)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ref-text", default=None, help="Transcript of the reference audio; recommended for clone quality.")
    return parser
