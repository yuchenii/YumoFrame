...
while (index < source.length) {
  ...
  const periodEnd = char === '.' && (index + 1 === source.length || /\s/.test(source[index + 1] ?? '') || closers.has(source[index + 1] ?? ''));
  if (!hardEnd.has(char) && !periodEnd) {
    index += 1;
    continue;
  }
  index += 1;
  while (index < source.length && (hardEnd.has(source[index]) || closers.has(source[index]))) index += 1;
...
console.log(JSON.stringify({source: input, units: units.map((text, i) => ({id: `u${i + 1}`, text}))}, null, 2));
